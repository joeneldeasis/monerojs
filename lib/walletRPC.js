...
class walletRPC {
   ...
   *
   * @returns {promise} resolve, reject resolve, reject
  ...
  label_address(index, label) {
    return new Promise((resolve, reject) => {
      ...
      this._run('label_address', params)
      .then(result => {
        this.store() // Save wallet state after labelling address
        .then(() => {
          resolve(result);
        })
        .catch(err => {
   ...
   * @param {string} tag - Tag of account(s) to look up (optional)
   *
   * @returns {promise} resolve, reject
   * @resolves {object} - Example: {
   *   subaddress_accounts: {
  ...
  tag_accounts(accounts, tag) {
    return new Promise((resolve, reject) => {
      ...
      .then(result => {
        this.store() // Save wallet state after tagging account
        .then(() => {
          resolve(result);
        })
        .catch(err => {
   ...
   * @returns {promise} resolve, reject
   * @resolves {object} - Example: {
   *   notes: ['monerojs test suite transaction note']
   * }
   */
  get_tx_notes(txids) {
   ...
   * @returns {promise} resolve, reject
   * @resolves {object} - Example: {
   *   value: 'monerojs unit test suite wallet attribute'
   * }
   */
  get_attribute(key) {
    let params = { key: key };
   ...
   * @returns {promise} resolve, reject
   * @resolves {object} - Example: {
   *   uri: '"monero:9sykYqd8soGa9Fv8zDMdt2gN8z2Aj5qQeBNpXjxRowkyCoWCYxa3xumYQe5MmQJuFN5CVTQwK2gqtfBNsFqa16gp1L4uGBU&tx_amount=1.000000000000&recipient_name=monerojs%20testnet%20unit%20test%20suite%20wallet&tx_description=monerojs%20unit%20test%20suite%20uri'
   * }
   */
   ...
   *
   * @returns {promise} resolve, reject
   * @resolves {object} - Example: (multisignature wallet) {
   ...
   *
   * @returns {promise} resolve, reject
   * @resolves {object} - Example: (non-multisignature wallet) {
   *   multisig: false,
   *   ready: false,
   *   threshold: 0,
   *   total: 0
